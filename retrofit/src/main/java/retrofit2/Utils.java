...
final class Utils {
    ...
    static Class<?> getRawType(Type type) {
        ...
        if (type instanceof ParameterizedType) {
            ...
            // 我不确定为什么getRawType()返回Type而不是Class。 Neal也不是，但怀疑存在与嵌套类相关的一些病态案例。
    ...
    static Type getParameterUpperBound(int index, ParameterizedType type) {
        Type[] types = type.getActualTypeArguments(); // 获得泛型的具体化
        ...
        if (paramType instanceof WildcardType) {
            return ((WildcardType) paramType).getUpperBounds()[0]; // 获得泛型的上界
        }
        return paramType;
    }
